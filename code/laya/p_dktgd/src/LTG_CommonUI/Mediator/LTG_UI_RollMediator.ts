...
export default class LTG_UI_RollMediator extends BaseUIMediator<LTG_UI_Roll> {
    ...
    _OnShow() {
        ...
        if (!this._cacheData.onSpecial) {
            this._cacheData.onSpecial = this._cacheData.onRolled;
        }
        ...
        for (let i = 1; i <= 6; ++i) {
            ...
            if (data.text_icon_path) {
                itemUI.m_loader_title.url = data.text_icon_path;
            ...
            itemUI.m_selected.selectedIndex = 0;
        }
    ...
    private _DoRoll() {
        ...
        LTUI.LockScreen();
        this.ready();
        AudioManager.instance.PlayAudioByPath(PackConst.data.audio_roll);
        ...
        Laya.Tween.to(this.ui.m_view.m_view_roll, { rotation: targetRotate }, 2100, Laya.Ease.quadOut,
    ...
    private rolledEnd(endIndex: number) {
        let itemName = 'm_item_0' + endIndex.toFixed(0);
        let itemUI = this.ui.m_view.m_view_roll[itemName] as LTG_UI_item_view_roll;
        itemUI.m_selected.selectedIndex = 1;
    }
    private _OnRollEnd() {
        ...
        this.rolledEnd(this._cacheFinalIndex);
        this._UpdateUI();
        LTUI.UnlockScreen();
