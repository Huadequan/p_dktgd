...
export class P2Manager {
    ...
    public InitWorld(gravity: number[]) {
    ...
    public CreateStaticBox(size: Laya.Vector3, pos: Laya.Vector3, yaw: number, isXZ: boolean = true): p2.Body {
        ...
        body.addShape(shape);
        body.type = p2.Body.STATIC;
        body.angle = yaw * (isXZ ? -MathEx.Deg2Rad : MathEx.Deg2Rad);
        return body;
    }
    ...
    public SyncTransFromRigXY(trans: Laya.Transform3D, rig: p2.Body) {
        let pos = rig.position;
        this._cachePos.setValue(pos[0], pos[1], trans.position.z);
        trans.position = this._cachePos;
    }
